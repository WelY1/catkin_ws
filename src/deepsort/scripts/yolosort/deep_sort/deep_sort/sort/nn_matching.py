...
def _nn_cosine_distance(x, y):
    ...
    ndarray
        ...
        #取每列的最小值，一列就是一个det对象和各个track对象的代价，
        # 这样相当于计算出了每个det对象跟这个track对象所有特征计算出来的最小代价值，
        # 也就是表示每个det对象和track对象的最小代价。
...
class NearestNeighborDistanceMetric(object):
    ...
    对于每个目标，返回最近邻的距离度量, 即与到目前为止已观察到的任何样本的最接近距离。
    ...
    def partial_fit(self, features, targets, active_targets):
        ...
        for feature, target in zip(features, targets):
            ...
            # samples字典    {target_1:[feature_1, feature_2, ..., feature_budget], target2:[], ...}
            self.samples.setdefault(target, []).append(feature)
            if self.budget is not None:
                # 只保存budget个目标，保存最新的（从后往前），前面的就 删掉了
    ...
    def distance(self, features, targets):
        ...
        计算features和targets对应的features之间的距离，返回一个成本矩阵（代价矩阵）
        """
        cost_matrix = np.zeros((len(targets), len(features)))
        for i, target in enumerate(targets):
            cost_matrix[i, :] = self._metric(self.samples[target], features)
            #算余弦距离，这里的samples就是目前所有confirmed对象的特征。
            # 这里其实是找到了每个confirmed对象和相似度最高的那个检测对象的代价，
            # 比如有5个跟踪对象，4个检测对象，那么这里返回的矩阵就是5*4，其中元素i，j表示第i个跟踪对象和第j个检测对象之间的代价。
            # 所以这里每次多一行，一行就表示该track对象和每个det的最小代价
        return cost_matrix
