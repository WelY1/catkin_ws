...
def matching_cascade(
    ...
    for level in range(cascade_depth):
        # 如果没有detections，退出循环
        if len(unmatched_detections) == 0:  # 当所有的检测框都匹配上之后结束匹配
            break

        # 当前level的所有tracks索引
        # 步骤6：Select tracks by age
        track_indices_l = [                                                            # time_since_update表示距离上次更新过去的帧数，每次预测后+1，
            k for k in track_indices                                               # 每次更新后置为0，长时间不更新（如被遮挡或离开画面）会一直增加，大于Age_max则被删除
            if tracks[k].time_since_update == 1 + level      # level越小表示对象越新，优先匹配更新的track，
        ]                                                                                               # 可以理解为先匹配连续轨迹然后匹配有间断但为达到max_age的对象
        # 如果当前level没有track，继续
