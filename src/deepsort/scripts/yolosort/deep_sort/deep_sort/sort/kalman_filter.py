...
class KalmanFilter(object):
    ...
    def __init__(self):
        ...
        self._motion_mat = np.eye(2 * ndim, 2 * ndim)             # 状态转移矩阵A[]8*8
        for i in range(ndim):
    ...
    def initiate(self, measurement):
        ...
        mean = np.r_[mean_pos, mean_vel]            # 按列连接两个矩阵，要求列数相等
        ...
        std = [
            ...
            1e-5,
            10 * self._std_weight_velocity * measurement[3]]
        covariance = np.diag(np.square(std))           # 平方
    ...
    def predict(self, mean, covariance):
        ...
        # Update time state x' = Ax (1)
        # x为track在t-1时刻的均值，F称为状态转移矩阵，该公式预测t时刻的x'
        # self._motion_mat为A_k是作用在 x_{k-1}上的状态变换模型
        mean = np.dot(self._motion_mat, mean)
        # Calculate error covariance P' = APA^T+Q (2)
    ...
    def update(self, mean, covariance, measurement):
        ...
        """
        # 将均值和协方差映射到检测空间，得到 Hx'和S=HPH^T+R
        ...
        # 所以代码中把公式两边同时乘上S，右边的S*S的逆变成了单位矩阵，转化成AX=B形式求解。      S^T K^T =(PH^T)^T
        ...
        innovation = measurement - projected_mean                  # y = z - Hx

        # 更新后的均值向量 x = x' + Ky (6)
        new_mean = mean + np.dot(innovation, kalman_gain.T)       # x = x + Ky
