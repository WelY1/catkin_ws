"""
use yolov5s-TensorRT to detect license plate.
"""
import ctypes
import os
import random
...
class YoLov5TRT(object):
    ...
    def __init__(self):
        ...
        PLUGIN_LIBRARY = RosPack().get_path('camera') + '/scripts/yolosort/lp/libmyplugins.so'
        ctypes.CDLL(PLUGIN_LIBRARY)
        engine_file_path = RosPack().get_path('camera') + "/scripts/yolosort/lp/yolov5n.engine"
    ...
    def infer(self, img_batch):
        ...
        start11 = time.time() 
        for i, image_raw in enumerate(img_batch):
        ...
        if (output[0] > 0):
            ...
            for i in range(self.batch_size):
                box = self.post_process(
                    output[i * 6001: (i + 1) * 6001], batch_origin_h[i], batch_origin_w[i]
                )
        time13 = time.time() - start13
        return box
    ...
    def post_process(self, output, origin_h, origin_w):
        ...
        param:
            ...
            origin_w:   width of original image
        return:
            a tuple of (x1, y1, x2, y2, result_scores, result_classid)
        """
        ...
        if len(boxes):
            xx1,yy1,xx2,yy2 = int(boxes[0, 0]),int(boxes[0, 1]),int(boxes[0, 2]),int(boxes[0, 3])
            # result_boxes = boxes[0, :4]
            result_scores = boxes[0, 4]
            result_classid = int(boxes[0, 5]) 
            return (xx1, yy1, xx2, yy2, result_scores, result_classid)
        else:
            return None
...
if __name__ == "__main__":
    # from recognitionpth import Recognition
    # load labels
    categories = ["lp"]
    ...
    # avg24 = 0
    try:
        image_dir = "/home/zxc/yutong/lp_det_ocr_TensorRT/samples/*.jpg"
        imgs = glob.glob(image_dir)
        for img in imgs:
            box,time11,time12,time13 = yolov5_wrapper.infer([cv2.imread(img)])
        for img in imgs:
            
            im = cv2.imread(img)
            start14 = time.time()
            box,time11,time12,time13 = yolov5_wrapper.infer([im])
            time14 = time.time() - start14
            
            # if box is not None:
            #     lp_img = im[box[1]:box[3], box[0]:box[2]]
            #     start24 = time.time()
            #     lp, _,time21,time22,time23 = recognition.infer([lp_img]) 
